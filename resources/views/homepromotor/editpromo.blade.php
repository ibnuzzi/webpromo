...
<head>
    ...
    <title>Edit Promo - GudangPromo</title>
...
<body>
                ...
                <div class="col-lg-12">
                    <div class="account-card">
                        <div class="account-title">
                            <h3 style="margin-left: 40%;">Edit Promomu</h3>
                        </div>
                        <div class="account-content">
                            <div class="row">
                                @if ($data->status == 0)
                                    <div class="alert alert-warning alert-dismissible fade show mb-2" role="alert">
                                        Tunggu Persetujuan Dari Admin Agar Promo Anda Dapat Dipublis Kehalaman
                                        Guest
                                    </div>
                                ...
                                @else
                                    <div class="alert alert-success alert-dismissible fade show mb-2" role="alert">
                                        Promo Anda Sudah Disetujui Oleh Admin
                                    </div>
                                @endif
                                <form action="/updatepromo/{{ $data->id }}" method="post"
                                    ...
                                    <div class="col-md-6 col-lg-12">
                                        <div class="form-group"><label class="form-label">Deskripsi Promo</label>
                                            <textarea class="form-control" name="deskripsipromo" id="Mysummernote" cols="30" rows="10"
                                                value="{{ $data->deskripsipromo }}">{{ $data->deskripsipromo }}</textarea>
                                        </div>
                                    </div>
                                    @error('deskripsipromo')
                                        <div class="alert alert-danger mb-2 mt-2"> {{ $message }} </div>
                                    @enderror
                                    {{-- direlasikan --}}
                                    <div class="col-md-6 col-lg-12">
                                        <div class="form-group"><label class="form-label">Kategori Promo</label>
                                            <select name="kategoripromo" class="form-control">
                                                <option value="{{ $data->kategoripromo }}">Pilih</option>
                                                @foreach ($data1 as $k)
                                                    <option value="{{ $k->id }}" <?php if ($data->kategoripromo == $k->id) {
                                                        echo 'selected';
                                                    } ?>>
                                                        {{ $k->kategori }}</option>
                                                @endforeach
                                        ...
                                        <div class="upload__box">
                                            <div class="upload__btn-box">
                                                <label class="upload__btn">
                                                    {{-- <img src="{{asset('/fotopromo/' . $data->sampul)}}}}" alt="" srcset=""> --}}
                                                    <img style="height: 200px;"
                                                        src="{{ asset('sampul/' . $data->sampul) }}"
                                                        alt="product" /></a>
                                                    <p class="mb-0">Masukkan Sampul produk anda</p>
                                                    <input name="sampul" type="file" data-max_length="20"
                                                        class="upload__inputfile" value="{{ $data->sampul }}">
                                                </label>
                                            </div>
                                            @error('sampul')
                                                <div class="alert alert-danger mb-2 mt-2"> {{ $message }} </div>
                                                        ...
                                                        <div class="product-card">
                                                            <center>
                                                                <div class="product-media">
                                                                    ...
                                                                    <img style="height: 190px; width:190px;"
                                                                        src="{{ asset('/fotopromo/' . $gambar->fotoproduk) }}"
                                                                        alt="product">
                                                            ...
                                                            <div class="product-content">
                                                                ...
                                                                {{-- Modal --}}
                                                                <div class="">
                                                                    <a class="editBtn btn btn-primary ms-2 mb-1"
                                                                        data-bs-toggle="modal"
                                                                        data-data="{{ $gambar }}"
                                                                        data-bs-target="#promo-edit ">Ubah
                                                                        Foto</a>
                                                                </div>
                                                                {{-- end --}}

                                                                <a id="form-delete" class="deletefoto"
                                                                    href="{{ Route('deletefotoproduk', ['id' => $gambar->id]) }}"
                                                                    type="button"
                                                                    data-nama="{{ $gambar->fotoproduk }}">
                                            ...
                                            @if ($errors->has('fotoproduk'))
                                                    <div class="alert alert-danger mb-2">
                                                        <ul>
                                                            @foreach ($errors->get('fotoproduk') as $error)
                                                                <li>{{ $error }}</li>
                                                            @endforeach
                                                        </ul>
                                                    </div>
                                            ...
                                            <div class="col-lg-5">
                                                @if ($data->status == 2)
                                                    <button type="submit" class="btn btn-warning">Kirim
                                                        Promo</button>
                                                @else
                                                ...
                                                <button type="button" class="btn btn-success"
                                                    data-bs-target="#tambah-promo" data-bs-toggle="modal">Tambah Foto
                                                    Produk</button>
                                                {{-- End --}}
                                            </div>
                                        </div>
    ...
    <footer class="footer-part">
        ...
        {{-- Modal Edit --}}
        <div class="modal fade" id="promo-edit">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content"><button class="modal-close" data-bs-dismiss="modal"><i
                            class="icofont-close"></i></button>
                    <form id="form-update" action="" method="post" class="modal-form"
                        enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="form-title">
                            <h3>Edit Foto Promo</h3>
                        </div>
                        <img id="nama" src="{{ asset('/fotopromo/' . $data->fotoproduk) }}" class="mb-2"
                            style="width: 350px" alt="">
                        <div class="form-group"><label class="form-label">Foto Promo</label><input
                                class="form-control" name="fotoproduk" type="file"></div>
                        <button class="form-btn" type="submit">Simpan Foto</button>
                    </form>
                </div>
            </div>
        </div>
        {{-- End --}}
        {{-- Modal Tambah --}}
        <div class="modal fade" id="tambah-promo">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content"><button class="modal-close" data-bs-dismiss="modal"><i
                            class="icofont-close"></i></button>
                    <form id="form-insert" action="/insertfotoproduk" method="post" class="modal-form"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-title">
                            <h3>Tambah Foto Promo</h3>
                        </div>
                        <div class="form-group"><label class="form-label">Foto Promo</label>
                            <input name="fotoproduk" class="form-control" type="file">
                        </div>
    ...
    <script>
        $('.editBtn').click(function(e) {
            ...
            let nama = $('#nama').attr('src', "{{ asset('/fotopromo/' . ':foto') }}".replace(':foto', data
                .fotoproduk));

            $('#form-delete').attr('action', "{{ route('deletefotoproduk', ':id') }}".replace(":id", data.id))

            $('#form-update').attr('action', "{{ route('editfotoproduk', ':id') }}".replace(":id", data.id))

            $('#form-insert').attr('action', "{{ route('insertfotoproduk', ':id') }}".replace(":id", data.id))
        })
    </script>
</body>
