...
<head>
    ...
    <style>
        ...
        .product-wish[data-status="0"] path {
            fill: #ccc;
        }

        .product-wish[data-status="1"] path {
            fill: #f77737;
        }
    </style>
</head>
...
<body>
    ...
    <div class="mobile-menu"><a href="index.html" title="Home Page"><i
            ...
            title="Wishlist"><i class="fa-regular fa-bookmark"></i></i><span>wishlist</span><sup>0</sup></a><a
            href="compare.html" title="Compare List"><i
                            ...
                            <div class="col">
                                <div class="product-card">
                                    <div class="product-media">
                                        <form id="promo-form-{{ $row->id }}" method="POST"
                                            onsubmit="savePromo({{ $row->id }}); return false;"
                                            action="{{ route('unbookmark', ['id' => $row->id]) }}">
                                            @csrf
                                            <button type="submit" class="product-wish wish"
                                                data-status="{{ $row->bookmark }}">
                                                <svg style="color:#f77737;" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        ...
                                        <a class="product-image" href="/detpromo/{{ $row->produk_id }}">
                                            <img src="{{ asset('sampul/' . $row->sampul) }}" alt="product"
...
{{-- <script>
    ddocument.addEventListener('DOMContentLoaded', function() {
        var button = document.querySelector('.product-wish');
        var status = localStorage.getItem('bookmark');
        if (status == 1) {
            button.dataset.status = 1;
            button.querySelector('path').setAttribute('fill', 'black');
            button.style.color = 'black';
        }
    });

    function savePromo(id) {
        var form = document.getElementById('promo-form-' + id);
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    var button = form.querySelector('.product-wish');
                    var defaultColor = window.getComputedStyle(button, null).getPropertyValue('color');
                    if (button.dataset.status == 1) {
                        button.dataset.status = 0;
                        button.querySelector('path').setAttribute('fill', defaultColor);
                        localStorage.setItem('bookmark', 0);
                        alert('Promo berhasil dihapus dari Simpan');
                    } else {
                        button.dataset.status = 1;
                        button.querySelector('path').setAttribute('fill', 'black');
                        button.style.color = 'black';
        ...
        data.append('status', form.querySelector('.product-wish').dataset.status);
