                ...
                <div class="col-12 col-sm-10 col-md-12 col-lg-12 col-xl-10">
                    ...
                    <div class="user-form-card">
                        ...
                        <div class="user-form-group">
                            <form class="user-form" action="/loginproses" method="post">
                                @csrf
                                <div class="form-group"><input type="email" class="form-control"
                                        placeholder="Masukkan email kamu" name="email" /></div>
                                        @error('email')
                                        <div class="alert alert-danger mb-3"> {{ $message }} </div>
                                        @enderror
                                <div class="form-group"><input type="password" class="form-control"
                                        placeholder="Masukkan password kamu" name="password" /></div>
                                        @error('password')
                                        <div class="alert alert-danger mb-3"> {{ $message }} </div>
                                        @enderror
                                <div class="form-check mb-3"><input class="form-check-input" type="checkbox"
                                        value="" id="check" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }} /><label class="form-check-label"
                                        for="check">Ingat saya</label></div>
                                <div class="form-button">
                                    <button type="submit">Masuk</button>
                                    <p>Lupa kata sandi Anda?<a href="/lupapassword">ulang di sini</a></p>
                                </div>
                            </form>
                        </div>
                    </div>
